...
const ProblemTable = ({problems}) => {
    ...
    const allTags = useMemo(() => {
        ...
        return Array.from(tagSet)    // it will return an array of unique tags
    }, [problems])


// --------- search by title , filter by difficulty and tags logic -------------------
...
{/* Title and Create Playlist Button */}
            ...
            {paginatedProblems.length > 0 ? (
              paginatedProblems.map((problem) => {
                const isSolved = problem.solvedBy.some(
                  (user) => user.userId === authUser?.id      // checking current user id is in the solvedBy array or not. if yes then it is solved
                );
                  ...
                  <tr key={problem.id}>
                    ...
                    <td>
                      <div className="flex flex-col md:flex-row gap-2 items-start md:items-center">
                        ...
                        {/* if user is admin then show edit and delete buttons */}
                        {authUser?.role === "ADMIN" && (
